...
The local delta parser recognizes only `EXT-X-GAP`, `EXT-X-PROGRAM-DATE-TIME`, and `EXT-X-PART` as segment-scoped tags when finding media block boundaries (`src/m3u8_cache.rs`, lines 597-600). It does not implement special handling for `EXT-X-KEY`, `EXT-X-MAP`, `EXT-X-DISCONTINUITY`, `EXT-X-BITRATE`, `CAN-SKIP-DATERANGES`, or `RECENTLY-REMOVED-DATERANGES`.
...
The OBS LL-HLS example does emit `EXT-X-PRELOAD-HINT:TYPE=PART` (`/Users/jamie/wavey.ai/web-services/examples/obs-rist-llhls/src/main.rs`, lines 454-456) and serves hinted part paths through a blocking `get_part_blocking` loop that waits until the part is known, too old, too far ahead, or timed out (`examples/obs-rist-llhls/src/main.rs`, lines 460-479 and 683-689).

Verified implication: the main `playlists` plus `hls` path now has the core preload-hint surface for byte-ranged fMP4 parent segments. Remaining gaps are HTTP behavior for currently hinted unknown-length ranges and full delta-update state preservation for legal playlists outside our generated subset.

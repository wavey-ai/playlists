...
Recommendation: explicitly state whether `PART-TARGET` is immutable for a Media Playlist, and if it can change, define when clients must apply the new value. Local policy: generated playlists treat `PART-TARGET` as immutable and configured, not inferred from observed retained parts.
...
The HLS handler treats `_HLS_skip=YES` and `_HLS_skip=v2` identically. The `M3u8Cache` delta generator only requires `CAN-SKIP-UNTIL`, inserts one `EXT-X-SKIP`, and retains lines based on parsed media blocks.

The local delta parser now recognizes `EXT-X-BITRATE`, `EXT-X-BYTERANGE`, `EXT-X-DISCONTINUITY`, `EXT-X-GAP`, `EXT-X-KEY`, `EXT-X-MAP`, `EXT-X-PART`, and `EXT-X-PROGRAM-DATE-TIME` as segment-scoped/state tags when finding media block boundaries. It does not implement Date Range delta behavior for `CAN-SKIP-DATERANGES` or `RECENTLY-REMOVED-DATERANGES`.

Verified implication: byte-ranged fMP4 delta updates now retain the state needed by the first unskipped parent resource in the generated playlist shape. The implementation is still not a complete general-purpose delta-update merger/generator for playlists that use Date Ranges.
...
The `playlists` manifest builder now emits byte-ranged `EXT-X-PART` entries for known fMP4 part sizes, byte-ranged parent segment entries, and a live `EXT-X-PRELOAD-HINT:TYPE=PART` for the current parent segment using `BYTERANGE-START` with no speculative `BYTERANGE-LENGTH`. For discrete part resources, it emits a preload hint for the next `pN.mp4` resource.
...
Verified implication: the main `playlists` plus `hls` path now has the core preload-hint surface for byte-ranged fMP4 parent segments and discrete part resources. Remaining gaps are multi-part unknown-length range streaming semantics and full delta-update state preservation for legal playlists outside our generated subset.
...
### I. Part target and hold-back are configured and stable

The manifest builder now uses `Options::part_target_ms` for `EXT-X-PART-INF:PART-TARGET` and derives `PART-HOLD-BACK` from that configured value. It also emits explicit `HOLD-BACK` at three Target Durations.

Verified implication: this avoids changing `PART-TARGET`, `PART-HOLD-BACK`, or the server-side Advance Part Limit because of retained-window churn or early short parts. Callers must configure `part_target_ms` to be at least as large as the actual part durations they publish.

### J. Rendition reports are explicit opt-in state

The manifest builder can now emit `EXT-X-RENDITION-REPORT` entries through `RenditionReport` values attached to an `M3u8Manifest`.

Verified implication: the core tag is supported for multi-rendition LL-HLS media playlists, but the current one-stream `Playlists` API does not synthesize peer rendition reports automatically because it does not own a multivariant/rendition graph.
...
- Add `EXT-X-RENDITION-REPORT` for each peer rendition when a real multivariant presentation exposes multiple media playlists. The manifest builder supports this explicitly; the current single-stream path does not invent reports.
- Emit `EXT-X-PART-INF` whenever `EXT-X-PART` is present.
- Treat `PART-TARGET` as effectively immutable unless the final RFC/draft explicitly allows mutation. This is implemented for generated playlists through `Options::part_target_ms`.
- Include `EXT-X-PRELOAD-HINT` for live playlists with parts if the goal is compliance with the Low-Latency Server Configuration Profile. This is implemented for generated byte-ranged fMP4 playlists and generated discrete part-resource playlists.
